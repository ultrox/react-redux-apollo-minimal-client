...
const storage = globals.__SERVER__ ? null : require('localforage');

const host = clientUrl => (globals.__SERVER__ ? `http://${config.apiHost}:${config.apiPort}` : clientUrl);
...
export function createApp(req) {
    ...
    if (globals.__SERVER__ && req) {
        const app = configureApp(rest(host('/api'))
            .axios(axios.create({
                headers: {
